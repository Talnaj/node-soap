...
Client.prototype._defineMethod = function(method, location) {
  var self = this;
  var temp;
  return function(extraProperties, args, callback, options, extraHeaders) {
    ...
    self._invoke(extraProperties, method, args, location, function(error, result, rawResponse, soapHeader, rawRequest) {
      callback(error, result, rawResponse, soapHeader, rawRequest);
    }, options, extraHeaders);
  };
};

Client.prototype._invoke = function(extraProperties, method, args, location, callback, options, extraHeaders) {
  var self = this,
    name = method.$name,
    input = method.input,
  ...
  //Updated the encoding check, sometimes inputSoap is an object and the proerty "use" indicated its encoded or not
  //encodingStyle, default is soap, but sometimes should be instead of envelopeKey
  if ((style === 'rpc')&& ( ( input.parts || input.name==="element" ) || args === null) ) {
    assert.ok(!style || style === 'rpc', 'invalid message definition for document style binding');
    message = self.wsdl.objectToRpcXML(name, args, alias, ns,(input.name!=="element" ));
    ((method.inputSoap === 'encoded') || (method.inputSoap.use === 'encoded')) && (encoding = envelopeKey + ':encodingStyle="http://schemas.xmlsoap.org/soap/encoding/" ');
  } else {
